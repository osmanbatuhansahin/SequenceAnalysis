...
gene_expression[1:3, c("FPKM.control1","FPKM.control2","FPKM.control3","FPKM.controlRep3")]
...
i = row.names(gene_expression) == "ENSG00000198431"
gene_expression[i,]

#What if we want to view values for a list of genes of interest all at once? 
#genes_of_interest = c("TST", "MMP11", "LGALS2", "ISX")
genes_of_interest = c("ENSG00000198431","ENSG00000136244","ENSG00000110090","ENSG00000170345")
...
min(gene_expression[,"FPKM.control1"])
max(gene_expression[,"FPKM.control1"])
...
data_columns=c(1:10)
short_names=c("control1","control2","control3","control4","control5","palmitate1","palmitate2","palmitate3","palmitate4","palmitate5")
...
x = gene_expression[,"FPKM.control1"]
y = gene_expression[,"FPKM.control2"]
plot(x=log2(x+min_nonzero), y=log2(y+min_nonzero), pch=16, col="blue", cex=0.25, xlab="FPKM (control, Replicate 1)", ylab="FPKM (control, Replicate 2)", main="Comparison of expression values for a pair of replicates")
...
smoothScatter(x=log2(x+min_nonzero), y=log2(y+min_nonzero), xlab="FPKM (control, Replicate 1)", ylab="FPKM (control, Replicate 2)", main="Comparison of expression values for a pair of replicates", colramp=colors, nbin=200)
...
plotCor("FPKM.control1", "FPKM.palmitate1", "control1 vs palmitate1", "tomato2")
plotCor("FPKM.control2", "FPKM.palmitate2", "control2 vs palmitate2", "royalblue2")
...
par(mfrow=c(1,2))
plotCor2("FPKM.control1", "FPKM.palmitate1", "control1 vs palmitate1", "tomato2")
plotCor2("FPKM.control2", "FPKM.palmitate2", "control2 vs palmitate2", "royalblue2")


#### Compare the correlation 'distance' between all replicates
#Do we see the expected pattern for all eight libraries (i.e. replicates most similar, then control vs. palmitate)?
...
#What pattern do we expect to see, given the types of libraries we have (technical replicates, biologal replicates, palmiitete/control)?
d=1-r
...
hist(results_genes[sig,"de"], breaks=50, col="seagreen", xlab="log2(Fold change) control vs palmitate", main="Distribution of differential expression values")
...
plot(x=x, y=y, pch=16, cex=0.25, xlab="control FPKM (log2)", ylab="palmitate FPKM (log2)", main="control vs palmitate FPKMs")
abline(a=0, b=1)
xsig=x[sig]
ysig=y[sig]
